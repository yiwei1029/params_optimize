...
def loss_func(x_arr, y):  # 合成函数和样本y的差距损失
    ...
    return lambda args: sum(
        ((np.array([overlay_funcs(x_arr)(args[i], args[i + 1], args[i + 2], args[i + 3], args[i + 4])
...
if __name__ == '__main__':
    ...
    # args = np.array([5, 5, 250, 50, 0, 5, 5, 250, 50, 0])  # rn,rg 初始值
    ...
    y = arr[:, 1]
    # print(arr)
    res = minimize(loss_func(x_arr, y), args, method='TNC')
