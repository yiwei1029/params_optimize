...
def cal_v(rn,rg,x):
    ...
    '''
    return rn+rg*x
def overlay_funcs(x_arr): #合成函数
    return lambda rn,rg:[(lambda rn,rg: cal_v(rn,rg,x))(rn,rg) for x in x_arr]
def loss_func(x_arr,y): #合成函数和样本y的差距损失
   return lambda args : sum((np.array(overlay_funcs(x_arr)(args[0],args[1]))-y)**2)
if __name__ == '__main__':
    # print(overlay_funcs([1,1])(1,2))
    # print(loss_func([1,2,3,4],[0,0,0,1])([1,2]))
    args=np.array([1,2]) #rn,rg 初始值
    x_arr = [1,2,3,4]
